 ...
 * `opAssign`, `S.opAssign` is going to be generated, but marked with `@disable`
 *
 * If `S` defines a postblit, a copy constructor, a move constructor,
 ...
 *---
 *
 * If `S` does not have a postblit, a copy constructor, a move constructor,
 * or a destructor, but contains at least one field that defines an `opAssign`
 * function (which is not disabled), then the body will make member-wise
 * assignments:
 *
 *---
 *this.field1 = s.field1;
...
private FuncDeclaration buildCopyOpAssign(StructDeclaration sd, Scope* sc, int op = OpAssign.copy)
{
    ...
    foreach (v; sd.fields)
    {
        ...
        if (tv.ty != Tstruct)
            continue;
        StructDeclaration sdv = (cast(TypeStruct)tv).sym;
        stc = mergeFuncAttrs(stc, hasOpAssign(sdv, sc, op));
    }
    ...
    DtorDeclaration dtor; // user-defined destructor
    foreach (dd; sd.dtors)
    {
        if (!dd.generated)
            dtor = dd;
    ...
    if (dtor || ctor && !ctor.generated || op == OpAssign.copy && postblit)
    {
        // if the type is not assignable, we cannot generate opAssign
        if (!sd.type.isAssignable()) // https://issues.dlang.org/show_bug.cgi?id=13044
            return null;
    ...
    const rval = op != OpAssign.move ? 0 : global.params.rvalueType ? STC.rvaluetype : STC.rvalueref;
    const paramStc = STC.wild | STC.ref_ | rval;
    const funcStc = stc | STC.ref_ | STC.return_;
    auto fop = generateOpAssignDeclaration(sd, paramStc, funcStc);
    if (!(stc & STC.disable))
        fop.fbody = generateCopyOpAssignBody(sd, ctor, dtor, postblit, op);
 ...
 *
 * If struct defines a destructor, copy or move constructor, or a posblit,
 ...
 *---
 *
 * Alternatively, member-wise assignments are performed:
 *
 *---
 *this.field1 = s.field1;
 *this.field2 = s.field2;
 ...
 * Params:
 *      sd = struct
 *      dtor = user-defined destructor
 *      ctor = copy or move constructor (generated or user-efined)
 *      pblt = user-defined postblit
 *      op = copy or move opAssign
 * Returns:
 *      generated function.
 */
private Statement generateCopyOpAssignBody(StructDeclaration sd, FuncDeclaration ctor, FuncDeclaration dtor, FuncDeclaration pblt, int op = OpAssign.copy)
{
    Loc loc; // internal code should have no loc to prevent coverage
    Expression e;
    if (dtor || ctor && !ctor.generated || op == OpAssign.copy && pblt)
    {
        /* this.dtor(); */
        if (sd.dtor)
        {
        ...
        /* this = p; */
        const bool blit = op == OpAssign.copy && sd.postblit || ctor is null;
        if (blit)
        {
            Expression ebl = new BlitExp(loc, new ThisExp(loc), new IdentifierExp(loc, Id.p));
            Expression epb;
            if (op == OpAssign.copy && sd.postblit)
            {
                /* this.postblit(); */
