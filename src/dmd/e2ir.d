...
bool ISREF(Declaration var)
{
    return (var.isParameter() &&
            ((I64 && var.type.isTypeStruct() && !var.type.isTypeStruct().sym.isPOD()) ||
    ...
    if (arguments && arguments.dim)
    {
        ...
        foreach (const i, arg; *arguments)
        {
            ...
            if ((config.exe == EX_WIN64 && arg.type.size(arg.loc) > REGSIZE && op == NotIntrinsic) ||
                (irs.params.is64bit && arg.type.isTypeStruct() && !arg.type.isTypeStruct().sym.isPOD()))
            {
            ...
            if (s.Sclass == SCauto || s.Sclass == SCparameter || s.Sclass == SCshadowreg)
            {
                if (fd && fd != irs.getFunc())
                {
                    ...
                    if (se.op == TOK.variable)
                        e = el_una(OPind, TYnptr, e);
                    if (ISREF(se.var) && (config.exe != EX_WIN64 || !(ISX64REF(se.var)) && v && v.offset && !forceStackAccess))
