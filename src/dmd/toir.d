...
elem *setEthis(const ref Loc loc, IRState *irs, elem *ey, AggregateDeclaration ad, bool setthis2 = false)
{
    ...
    FuncDeclaration thisfd = irs.getFunc();
    int offset = 0;
    Dsymbol adp = setthis2 ? ad.toParent2(): ad.toParent4();     // class/func we're nested in
    ...
    else if (thisfd.vthis && !thisfd.vthis2 &&
...
void buildCapture(FuncDeclaration fd)
{
    ...
    if (fd.closureVars.dim && !fd.needsClosure)
    {
        ...
        foreach (v; fd.closureVars)
        {
            ...
            if (fd.vthis)
                soffset -= toSymbol(fd.vthis2 ? fd.vthis2 : fd.vthis).Soffset; // see toElem.ToElemVisitor.visit(SymbolExp)
